HOMEWORK 9: MINIBLAST


NAME:  < Michael Levine >


COLLABORATORS AND OTHER RESOURCES:
List the names of everyone you talked to about this assignment
(classmates, TAs, ALAC tutors, upperclassmen, students/instructor via
LMS, etc.), and all of the resources (books, online reference
material, etc.) you consulted in completing this assignment.

< Herta Calvo-Faugier, Edwin Wallis, stackoverflow.com, Dan McCrevan>

Remember: Your implementation for this assignment must be done on your
own, as described in "Academic Integrity for Homework" handout.



ESTIMATE OF # OF HOURS SPENT ON THIS ASSIGNMENT:  < 5 >


HASH FUNCTION DESCRIPTION
Hash function goes through the each character, apply the xor operator(^) to the char, then multiplies by a seed value, then returns the big number mod size of table to get a number in the range of the index for the vector.






HASH TABLE IMPLEMENTATION
size_t Hashtable::getHash(std::string kmer)
{
    //const size_t initial = 2166136261U;
    const size_t seed = 16777619;
    //unsigned long hash = initial;
    unsigned long hash = 2166136261;
    for(size_t i = 0; i < kmer.length(); i++)
    {
        hash = hash ^ (kmer[i]); //xor  the low 8 bits
        hash = hash * seed; // multiply by the seed number
    }
    return hash % table_size;
}






ANALYSIS OF PERFORMANCE OF YOUR ALGORITHM:
(order notation & concise paragraph, < 200 words)

L = length of the genome sequence
q - query length
p - number of different locations where key is found
k - key size (k-mer size)

How much memory will the map data structure require (order notation for memory use)?
O(L-k)
This is the memory because there is you are storing L-k kmers in the hash_table

What is the order notation for performance (running time) of each of
the commands?
The order notation for query is O(p*q) because it goes through list of each location a kmer for a specific query and then goes through the entire query 


EXTRA CREDIT
Add a new command to implement the database using one of the other
 data structures that we have covered so far in the course: 
vectors, lists, arrays etc. 

Compare the performance your alternative method to the homework method
by making a table of run times for each of the genomes and query 
sets provided with the homework  and compare the times to build
the index and the times to process the queries. 
Document any new commands you have added in your README file.



MISC. COMMENTS TO GRADER:  
(optional, please be concise!)


