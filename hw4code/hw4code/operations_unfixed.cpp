...
bool file_operations(int argc, char** argv, char*& returned_buffer,
    ...
    char* buffer = new char[length];//bug#4
...
int pythagoras(int x, int y) {
    ...
    // no triple exists
                 //BUG #1 didnt have a default return if no triple exists
}
...
int vector_operations() {
    ...
    for(uint i = 0; i < all.size(); i+1) {//Bug#2
        if(i % 3 == 0) {
    ...
    for(uint i=counter-1; i >= 0; --i) {//bug #3
