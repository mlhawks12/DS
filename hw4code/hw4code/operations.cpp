...
int list_operations() {
  ...
  for(std::list<std::string>::iterator ritr = not_fruits.begin();
      ritr != not_fruits.end(); ++ritr) {
    fruit_itr = std::find(fruits.begin(), fruits.end(), *ritr);
      if(fruit_itr != fruits.end())
      {
...
int array_operations() {
  ...
  for(int x = 0; x < size; ++x) {
    ...
    for(int y = 0; y < size; ++y) {
      int tmp = *(tmp_ptr2+y);
      // pad single-d   igit numbers with a space so it looks nice
...
int vector_operations() {
  ...
  assert(v3sum == 0);
  assert(v3[10] == 0);
  for(int i=0; i<=v3.size()/2; ++i) { assert(v3[i] <= 0); }
